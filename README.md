...
Android project files under `src-tauri/gen/` can be regenerated by `tauri android init`.
To keep release cleartext behavior (`http://ip:port`servers access) consistent for source builders, use the provided scripts:

```bash
npm run mobile:android:init
npm run mobile:android:build:apk # or aab
```

Notes:
- `mobile:android:init` runs Android init and then reapplies the release `usesCleartextTraffic` policy.
- `mobile:android:build:aab` reapplies the policy before building.
- Set `ODLS_ANDROID_RELEASE_CLEARTEXT=false` to force release builds back to cleartext-disabled mode.
